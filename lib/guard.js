 ...
 */
export function defaultRules({ dshHome }) {
  const pluginData = join(homedir(), '.claude', 'plugins', 'data', 'dsh-deepseek-dsh')
  return [
 ...
 */
export function createGuard({ dshHome, extraProtectedPaths = [] }) {
  const rules = defaultRules({ dshHome })
