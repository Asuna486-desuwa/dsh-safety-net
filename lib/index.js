...
export function apply(ctx) {
  ...
  const guard = createGuard({
    dshHome: config.dshHome ?? dshHome(),
  ...
  ctx.safetyNet = {
    ...
    backups: null,          // wired in a later task with real fs adapter
    protectedSources: [],   // wired in a later task
    readSource: async () => { throw new Error('not wired yet') },
  }
  registerAll(ctx)
  ctx.effect(() => {}, 'safety-net: commands')
  ctx.effect(() => {}, 'safety-net: strict policy')
}
