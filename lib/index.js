 ...
 * `extraProtectedPaths` are walked too (review round 3, #1): user-added
 * protected paths must be backed up and checked by repair, not just blocked.
 */
async function expandSources(rules, { excludeRoot, extraProtectedPaths = [] }) {
  const out = []
  ...
  for (const extra of extraProtectedPaths) {
    try {
      const st = await stat(extra)
      if (st.isDirectory()) await walk(extra)
      else if (st.isFile()) out.push(extra)
    } catch {
      // missing path: skip
    }
  }
...
export function apply(ctx, pluginConfig) {
  // Cordis passes this plugin's own config as the second arg to apply().
  // Reading `ctx.config` directly throws "cannot get property config without
  // inject" in this cordis version, so take the injected config parameter.
  ...
  // race where the arrays were still [] when a command ran). extraProtectedPaths
  // are included so user-added paths are backed up / repaired, not just blocked
  // (review round 3, #1).
  const extraProtected = Array.isArray(config.extraProtectedPaths) ? config.extraProtectedPaths : []

  // The service object is built up-front so the `ready` closure can mutate its
  // own `protectedSources` field. In this cordis version `ctx.safetyNet = {...}`
  // throws "cannot set property without provide" — services must be registered
  // via ctx.provide(name, value); plain-object field mutation afterwards is fine.
  const service = {
    guard,
    dshHome: home,
  ...
  // are included so user-added paths are backed up / repaired, not just blocked
  // (review round 3, #1).
  service.ready = expandSources(guard.rules, { excludeRoot: home, extraProtectedPaths: extraProtected }).then((sources) => {
    service.protectedSources = sources
    return sources
  })

  ctx.provide('safetyNet', service)
  ...
  ctx.on('fs/write-intent', async (target, actor, next) => {
    ...
    if (guard.isProtected(path)) {
      ...
      let backupNote = ''
      try {
        const text = await service.readSource(path)
