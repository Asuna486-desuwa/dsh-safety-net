...
//
// Five slash commands, registered via ctx.commands. They work even when the
...
//   /safety-net-approve   — one-time approval to write a protected path
//                           (wires the guard's approveOnce into the CLI; the
//                           guard was previously a dead API with no entry)
...
export function buildStatusText({ protectedRuleCount, backupCount, strict, dshHome }) {
  return [
    ...
    `- strict mode: ${strict ? 'on (declared read-only default; host enforcement depends on sandbox backend)' : 'off (workspace-write default)'}`,
    `- DSH home: ${dshHome}`,
  ].join('\n')
}
 ...
 *   index.js: ctx.safetyNet = { guard, backups, dshHome, strict, ready }
 */
export function registerAll(ctx) {
  ...
  ctx.commands.register({
    ...
    handler: async () => {
      ...
      // wait for the protected-source scan so a fresh load never backs up 0 files
      const sources = s.ready ? await s.ready : (s.protectedSources ?? [])
      let count = 0
      const failures = []
      for (const src of sources) {
        try {
          ...
          // ENOENT = file vanished since the scan: not an error, skip.
          // Anything else (permission, disk full, ...) must be surfaced —
          // a silent catch hides real backup failures (review round 2, #2).
          if (err?.code !== 'ENOENT') {
      ...
      if (failures.length > 0) {
        const head = failures.length > 3 ? failures.slice(0, 3).concat([`… and ${failures.length - 3} more`]) : failures
        return {
          kind: 'error',
          text: `safety-net: backed up ${count} file(s), ${failures.length} FAILED:\n${head.join('\n')}`,
        }
      }
  ...
  ctx.commands.register({
    ...
    handler: async (rawInput) => {
      ...
      if (id === '') {
        let list
        try {
          list = await s.backups.list()
        } catch (err) {
          return { kind: 'error', text: `safety-net: failed to list backups — ${err instanceof Error ? err.message : String(err)}` }
        }
  ...
  ctx.commands.register({
    ...
    handler: async () => {
      const s = state()
      // wait for the protected-source scan so repair checks the real set
      const sources = s.ready ? await s.ready : (s.protectedSources ?? [])
      const missing = []
      for (const src of sources) {
        try {
  ...
  ctx.commands.register({
    name: 'safety-net-approve',
    description: 'safety-net: one-time approval to write a protected path: /safety-net-approve <path>',
    handler: async (rawInput) => {
      const s = state()
      if (!s.guard) return { kind: 'error', text: 'safety-net: guard not available' }
      const path = rawInput.trim()
      if (path === '') {
        return { kind: 'error', text: 'safety-net: usage — /safety-net-approve <path>' }
      }
      // Use the pure rule check, NOT isProtected(): isProtected() consumes a
      // pending one-time approval (and returns false), so calling it here
      // would silently eat an approval granted by an earlier call and then
      // misreport "not protected" (review round 2, bug 1).
      if (!s.guard.isRuleProtected(path)) {
        return { kind: 'success', text: `safety-net: ${path} is not protected — nothing to approve` }
      }
      s.guard.approveOnce(path)
      return { kind: 'success', text: `safety-net: approved one-time write to ${path} — the next matching mutation will be allowed` }
    },
  })
}
